...
foreach($arConc as $i=>$value){
  ...
  $sumaIva=0;
  if (intval($Item[0])>0) {
      $cadena_xml.='    <cfdi:Concepto Cantidad="1" Unidad="Pieza" Descripcion="'.trim($Item[1]).'" ValorUnitario="'.trim($Item[11]).'" Importe="'.trim($Item[11]).'" ClaveProdServ="01010101" ClaveUnidad="H87">'."\r\n";
