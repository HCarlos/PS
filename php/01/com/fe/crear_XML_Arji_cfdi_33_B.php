...
$cadena_xml.='  <cfdi:Emisor Rfc="'.trim($rfc_emisor).'" RegimenFiscal="601" Nombre="'.$razon_social_emisor.'" />'."\r\n";
...
foreach($arConc as $i=>$value){
  ...
  $sumaIva=0;
  if (intval($Item[0])>0) {
      $cadena_xml.='    <cfdi:Concepto Cantidad="'.trim($Item[0]).'" Unidad="Pieza" Descripcion="'.trim($Item[1]).'" ValorUnitario="'.trim($Item[3]).'" Importe="'.trim($Item[11]).'" ClaveProdServ="01010101" ClaveUnidad="H87">'."\r\n";
