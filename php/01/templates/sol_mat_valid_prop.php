...
<div class="span7">
        <form id="frmSolMatDet2" class="form" role="form" >
                ...
                <table>
                    ...
                    <tr>
                        <td><label for="cantidad_autorizada" class="textRight">Cantidad</label></td>
                        <td>
                            <span class="add-on"><i class="icon-asterisk red"></i></span>
                            <input class="altoMoz wd62prc" name="cantidad_autorizada" id="cantidad_autorizada" type="text"  pattern="[-+]?[0-9]*[.,]?[0-9]+" required>
                        </td>
                    </tr>
                  
                    <tr>
                        <td><label for="observaciones_autorizacion" class="textRight">Observaciones</label></td>
                        <td>
                            <span class="add-on"><i class="icon-asterisk red"></i></span>
                            <textarea rows="4" cols="50" class="" id="observaciones_autorizacion" name="observaciones_autorizacion" ></textarea>                  
                            <span class="lbl"></span>
                        </td>
                    </tr>
...
jQuery(function($) {
    $(document).ready(function () {
        ...
        $("#frmSolMatDet2").on("submit",function(event){
            ...
            $.post(obj.getValue(0)+"data/",  { o:39, t:4, p:2, c:queryString, from:0, cantidad:0,s:'' },
                function(json){
                    if (json[0].msg=="OK"){
        ...
        function updateForm(){
            ...
            $.post(obj.getValue(0) + "data/", {o:39, t:86, c:idsolicituddematerialdetalle, p:10, from:0, cantidad:0,s:nc},
                function(json){
                    ...
                    $("#cantidad_autorizada").val(json[0].cantidad_autorizada);            
                    $("#observaciones_autorizacion").val(json[0].observaciones_autorizacion); 

                    $("#tituloPanel").html("Editando el Item: "+idsolicituddematerialdetalle);  
                    $("#cantidad_autorizada").focus();
