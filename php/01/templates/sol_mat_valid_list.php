...
<div  class="row-fluid">
	...
	<div class="widget-toolbar orage">
		<label>
			...
			<input id="isautorizadas" type="checkbox" class="ace ace-switch ace-switch-4 orange" />
			<span class="lbl"></span>
		</label>
				...
				<table aria-describedby="sample-table-2_info" id="sample-table-2" class="table table-striped table-bordered table-hover dataTable">
					...
					<thead>
						<tr role="row">
							...
							<th aria-label="fecha_solicitud: activate to sort column ascending" style="width: 80px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="2" role="columnheader" class="sorting">FECHA</th>
							<th aria-label="observaciones: activate to sort column ascending" style="width: 150px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="3" role="columnheader" class="sorting">OBSERVACIONES</th>
							<th aria-label="status: activate to sort column ascending" style="width: 80px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="4" role="columnheader" class="sorting">STATUS</th>
...
// alert(1);
	...
	function fillTable(){
		...
		var sts = $("#isautorizadas").is(':checked') ? 1 : 0;	

		var nc = "u="+localStorage.nc+"&sts="+sts;
		
		$.post(obj.getValue(0) + "data/", {o:34, t:88, c:nc, p:10, from:0, cantidad:0,s:''},
			function(json){
				if (json.length){
					$.each(json, function(i, item) {

						var pro = parseInt(item.status_solicitud_de_material,0) == 1 ? ' checked ' : ' ';
						...
						tB +=' ';
						tB +='						<a class="blue modSolMatEnc1Pro" href="#" id="modSolMatEnc1Pro-'+item.idsolicituddematerial+'-'+item.solicitante+'" data-rel="tooltip" data-placement="top" title="Ver Artículos">';
						...
						if (sts == 0){
							...
							tB +='						<label>';
							tB +='							<input name="vaut1-'+item.idsolicituddematerial+'" id="vaut1-'+item.idsolicituddematerial+'" class="ace ace-switch ace-switch-6 aceaut" type="checkbox" '+pro+' >';
					...
					$(".modSolMatEnc1Pro").on("click",function(event){
						...
						getPropSolMatEnc1(arr[1],arr[2]);
					});

					$(".aceaut").on("change",function(event){
						event.preventDefault();
						...
						var id0 = id[1];
						var val0 = $(this).is(':checked') ? 1 : 0;
						var nc = "user="+localStorage.nc+"&id="+id0+"&val="+val0;
	                    $("#preloaderPrincipal").show();
				        $.post(obj.getValue(0)+"data/",  { o:38, t:3, p:2, c:nc, from:0, cantidad:0,s:'' },
				            function(json){
				                if (json[0].msg=="OK"){
				                    $("#preloaderPrincipal").hide();
				                }
	...
	$("#isautorizadas").on("change",function(event){
		event.preventDefault();
		onClickFillTable();
	});


	function getPropSolMatEnc1(IdSolMatEnc0, Solicitante){
        $("#contentProfile").html("");
        $("#contentMain").hide(function(){
	        $("#preloaderPrincipal").show();
	        obj.setIsTimeLine(false);
	        var nc = localStorage.nc;
	        $.post(obj.getValue(0) + "valida_cat-sol-mat-asign_1/", {
				user: nc,
