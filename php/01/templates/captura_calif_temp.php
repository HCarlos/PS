	...
	function paintTable(){
		...
		$.each(arrItems, function(i, item) {
		    ...
		    var xTR = "<tr class='"+defcolorep+"'>";
				...
				console.log(item.num_lista);

				$.each(arrHD, function(j, item) {

			  		xTR += "<td class='colclick alineaTextoalaDerecha' id='col-"+k+'-'+item.idgrumatcon+"'></td>"; 
			  		console.log("col-"+k+'-'+item.idgrumatcon);
					if (j==arrHD.length-1){
							var vcal = cal>0?cal:'';
							var vina = nss>0?nss:'';
							var vcon = cnd>0?cnd:'';
							...
							// xTR += "<td class='textRight'><input type='text' name='ina-"+k+'-'+localStorage.eval0+"' id='ina-"+k+'-'+localStorage.eval0+"' class='calif' maxlength='3' tabindex='"+tNdxIna+"' value='"+vina+"'  disabled='disabled' ></td>";  
							xTR += "<td class='textRight'><input type='text' name='ina-"+k+'-'+localStorage.eval0+"' id='ina-"+k+'-'+localStorage.eval0+"' class='calif' maxlength='3' tabindex='"+tNdxIna+"' value='"+vina+"' ></td>";  
			...
			$.ajax({
              ...
              success:  function(json){
						...
						$.each(json, function(j, item) {
							var vval = parseInt(item.calificacion,0)>0?parseInt(item.calificacion,0):'';
	  						$("#col-"+item.idboleta+'-'+item.idgrumatcon).html("<input type='text' name='idbolpar-"+item.idbolpar+'-'+item.idgrumatcon+"' id='idbolpar-"+item.idbolpar+'-'+item.idgrumatcon+"'  value='"+vval+"' class='calif' maxlength='3'  >"); 
	  						// $("#col-"+item.idboleta+'-'+item.idgrumatcon).html("<input type='text' name='idbolpar-"+item.idbolpar+'-'+item.idgrumatcon+"' id='idbolpar-"+item.idbolpar+'-'+item.idgrumatcon+"'  value='"+vval+"' class='calif' maxlength='3'  disabled='disabled' >"); 
	  					});
