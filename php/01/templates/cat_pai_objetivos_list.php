		...
		<tbody>
			<tr>
				<td data-original-title="Add new row" id="add_grid-table" title="" class="ui-pg-button ui-corner-all">
					<div class="ui-pg-div" id="btnAddPO0" >
	...
	<div id="user-profile-1" class="user-profile row-fluid">
			<div id="tblPAICC0_wrapper" class="dataTables_wrapper" role="grid">
				...
				<table aria-describedby="tblPAICC0_info" id="tblPAICC0" class="table table-striped table-bordered table-hover dataTable">
					...
					<thead>
						<tr role="row">
							<th aria-label="idpaiobjetivo: activate to sort column ascending" style="width: 50px;" colspan="1" rowspan="1" aria-controls="tblPAICC0" tabindex="0" role="columnheader" class="sorting" >ID</th>
							...
							<th aria-label="objetivo: activate to sort column ascending" style="width: 350px;" colspan="1" rowspan="1" aria-controls="tblPAICC0" tabindex="1" role="columnheader" class="sorting">Objetivo</th>
...
jQuery(function($) {
	...
	function fillTable(){
		...
		$.post(obj.getValue(0) + "data/", {o:65, t:57, c:nc, p:54, from:0, cantidad:0,s:''},
			function(json){
				
					$.each(json, function(i, item) {
						tB +=' 			<tr class="odd">';
						tB +='';
						tB +='				<td>'+padl(item.idpaiobjetivo,4)+'</td>';
						tB +='				<td>'+item.area_disciplinaria+'</td>';
						tB +='				<td>'+item.descripcion_criterio+'</td>';
						tB +='				<td>'+item.objetivo+'</td>';
						tB +='				<td class="center">'+item.grado_pai+'</td>';
						tB +='				<td>';
						tB +='					<div class="hidden-phone visible-desktop action-buttons">';
						tB +='';
						tB +='						<a class="green modPAIO0Pro" href="#" id="idpaiobjetivo-'+item.idpaiobjetivo+'" >';
						tB +='							<i class="icon-pencil bigger-130"></i>';
						tB +='						</a>';
						tB +='	';
						tB +='						<a class="red delPAIO0" href="#"  id="delPAIO0-'+item.idpaiobjetivo+'" >';
					...
					$(".modPAIO0Pro").on("click",function(event){
					...
					$(".delPAIO0").on("click",function(event){
						event.preventDefault();
						...
						if (resp){
							...
							//alert(arr[1]);
							obj.setIsTimeLine(false);
				            $.post(obj.getValue(0) + "data/", {o:65, t:2, c:arr[1], p:52, from:0, cantidad:0, s:''},
				            function(json) {
				            		if (json[0].msg=="OK"){
										onClickFillTable();
				        			}else{
	...
	$("#btnAddPO0").on("click",function(event){
		event.preventDefault();
	...
	function getPropPAIO0(IdPAIO0){
        $("#contentProfile").empty();
        $("#contentMain").hide(function(){
	        $("#preloaderPrincipal").show();
	        obj.setIsTimeLine(false);
	        var nc = localStorage.nc;
	        $.post(obj.getValue(0) + "cat-pai-objetivos-prop/", {
				user: nc,
				idpaiobjetivo: IdPAIO0
	            },
	            function(html) {	                
	                $("#contentProfile").html(html).show('slow',function(){
		                $('#breadcrumb').html(getBar('Inicio, Objetivos PAI '));
	                });
	            }, "html");
        });
        return false;
...
/*
	...
	stream.on("servidor", jsNewPAIO0);
	function jsNewPAIO0(datosServer) {
		var ms = datosServer.mensaje.split("-");
		//alert(datosServer);
		//obj.setIsTimeLine(true);
		if (ms[1]=='PAI_OBJETIVOS') {
			onClickFillTable();
		}
	}
*/
