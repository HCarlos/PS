						...
						<table class="table table-striped detFac" id="tblSolMatDet2" >
							<thead>
							    <tr>
								    ...
								    <th class="tbl80W">FECHA SOLICITUD</th>
								    <th class="tbl100W"></th>
							    </tr>
							</thead>
							<tbody>
			...
			function(json){
				...
				var suma = 0;
	           	...
	           	$.each(json, function(i, item) {
							...
							var pro = parseInt(item.status_solicitud_de_materiales,0) == 1 ? ' checked ' : ' ';
							var color = parseInt(item.idcolor,0) > 0 ? ' | <strong>COLOR</strong>: <strong style="color:'+item.codigo_color_hex+'">'+item.color+'</strong>':'';
							var FA = parseInt(item.status_solicitud_de_materiales,0) == 0 ? item.fecha_solicitud :'';
							str =  '  <tr>';
							...
							str += '	    <td class="tbl20W textRight">'+accounting.formatMoney(item.importe_autorizado)+'</td>';
							str += '	    <td class="tbl200W">'+item.observaciones_autorizacion+'</td>';
							str += '	    <td class="tbl0W">'+FA+'</td>';
							str += '	    <td class="tbl120W">';
							str += '		<div class="action-buttons">';	
							str += '			<a class="green modObsAut0" href="#" id="modObsAut0-'+item.idsolicituddematerialdetalle+'" >';
							str += '				<i class="icon-pencil bigger-130"></i>';
							str += '			</a>';
							str += ' ';
							str += '			<label>';
							str += '				<input name="vaut0-'+item.idsolicituddematerialdetalle+'" id="vaut0-'+item.idsolicituddematerialdetalle+'" class="ace ace-switch ace-switch-6" type="checkbox" '+pro+' >';
				...
				if ($(".modObsAut0").length) {
					$(".modObsAut0").on("click", function(event) {
						event.preventDefault();
				...
				$(".ace").on("change",function(event){
                    ...
                    $("#preloaderPrincipal").show();
			        $.post(obj.getValue(0)+"data/",  { o:39, t:3, p:2, c:nc, from:0, cantidad:0,s:'' },
			            function(json){
			                if (json[0].msg=="OK"){
			                    $("#preloaderPrincipal").hide();
			                }
...
*/
	...
	function getPropSolMatDetObsAut0(IdSolMatEnc0){
        ...
        $("#contentProfile").hide(function(){
	        ...
	        obj.setIsTimeLine(false);
	        var nc = localStorage.nc;
			$.post(obj.getValue(0) + "valida-sol-mat-prop_1/", {
					user: nc,
