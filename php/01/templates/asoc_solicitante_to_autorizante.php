    <div class="row">
        ...
        <div class="span4">
            <div class="panel panel-success">
              <div class="panel-heading">Solicitantes</div>
              <div class="panel-body" style="height: 30em !important; ">
                    <select class="lstSolicitantes " name="lstSolicitantes" id="lstSolicitantes" multiple="multiple" style="width:100% !important; height: 95% !important;" > 
                    </select>
        ...
        <div class="span4">
            <div class="panel panel-primary">
              <div class="panel-heading">Autorizantes</div>
              <div class="panel-body" style="height: 30em !important; ">
                    <label for="selAutorizantes" class="lblH2cmb">Lista de Autorizantes </label>
                    <select name="selAutorizantes" id="selAutorizantes" size="1" style="width:100% !important;" > 
                    </select>
                   
                    <label for="lstProfDir" class="lblH2">Solicitantes Asignados:</label>
...
function getAutorizantes(){
    ...
    $("#selAutorizantes").html('');
    $.post(obj.getValue(0)+"data/", { o:1, t:37, p:0,c:nc,from:0,cantidad:0, s:"" },
        function(json){
           $.each(json, function(i, item) {
                $("#selAutorizantes").append('<option value="'+item.data+'"> '+item.label+'</option>');
            });
            getSolicitantes();    
        }, "json"
    );  
...
function getSolicitantes(){
    var nc = "u="+localStorage.nc;
    $.post(obj.getValue(0)+"data/", { o:1, t:36, p:0,c:nc,from:0,cantidad:0, s:"" },
        function(json){
            var nc;
           $.each(json, function(i, item) {
                //alert(item.label);
                nc = item.label.split("@");
                $("#lstSolicitantes").append('<option value="'+item.data+'"> '+item.label+'</option>');
            });
            $("#lbl02").html(commaSeparateNumber(json.length)+" Solicitantes")
...
function getProfDir(){
    var nc = "u="+localStorage.nc;
    $("#lstProfDir").empty();
    var y = $('select[name="selAutorizantes"] option:selected').val(); 
    $("#lstProfDir").empty();
    $.post(obj.getValue(0)+"data/", {o:2, t:6, p:0,c:nc,from:0,cantidad:0, s:y },
        function(json){
           $.each(json, function(i, item) {
                $("#lstProfDir").append('<option value="'+item.data+'"> '+item.label+'</option>');
            });
            $("#lbl01").html(commaSeparateNumber(json.length)+" Solicitantes");
        }, "json"
    );  
}
...
$("#chkAllGru").on("click",function(event){
    ...
    if (checked) {
        $("#lstSolicitantes option").each(function(){
...
$("#selAutorizantes").on("change",function(event){
...
$("#AddItem").on("click",function(event){
    ...
    var x = $('.lstSolicitantes option:selected').val();  
    var y = $('select[name="selAutorizantes"] option:selected').val(); 
    ...
    if (isDefined(x)){
        ...
        x='';
        $(".lstSolicitantes option:selected").each(function () {
                x += $(this).val() + "|";
          });
    ...
    $.post(obj.getValue(0)+"data/", { o:4, c:d, t:10, p:1, s:nc, from:0, cantidad:0 },
        function(json){
            //alert( json[0].msg);
...
$("#DeleteItem").on("click",function(event){
    ...
    $.post(obj.getValue(0)+"data/", { o:4, c:x, t:20, p:1, s:nc, from:0, cantidad:0 },
        function(json){
            //alert(json[0].msg)
            if (json.length<=0 && json[0].msg=="Error") { return false;}
                //getAutorizantes();
