...
class PDF_Diag extends PDF_Sector {
    ...
    function Header(){   
		...
		$this->Cell(70,$this->nFont,utf8_decode("LISTA DE CALIFICACIONES OFICIALES"),'LTRB',1,'C',true);
...
if ( count($result)>0  ){
	...
	foreach ($result as $j => $value) {
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial','',7);
		if ( round($result[$j]->promcalof,0) >= 9) {
			$pdf->SetFillColor(172);
			$vBool = true;
		} else if ( round($result[$j]->promcalof,0) < 6) {
		...
		$pdf->Cell(27,$pdf->nFont,round($result[$j]->bim0,0),'L',0,'C',$vBool);
		$pdf->Cell(27,$pdf->nFont,round($result[$j]->bim1,0),'L',0,'C',$vBool);
		$pdf->Cell(27,$pdf->nFont,round($result[$j]->bim2,0),'L',0,'C',$vBool);
		$pdf->Cell(27,$pdf->nFont,round($result[$j]->promcalof,0),'LR',1,'C',$vBool);
