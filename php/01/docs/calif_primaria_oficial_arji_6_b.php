...
foreach ($arrAlu as $i => $value) {
	...
	if ( count($result)>0  ){
		...
		//$pdf->Cell(190,$pdf->nFont,$caro[0]->observaciones_generales,'',1,'L');
		...
		$pdf->setX(0);
		$ar = explode(';',$Pos[0]->maestro);				
		$pdf->setY(  floatval($ar[1])  );
		$pdf->setX(  floatval($ar[0])  );
		$pdf->Cell(80,$pdf->nFont,strtoupper(utf8_decode($pdf->profesor)),'',0,'C');
		
		// $pdf->Cell(10,$pdf->nFont,'','',0,'L');
