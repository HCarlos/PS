...
class PDF_Diag extends PDF_Sector {
	...
	function FormatCal($cal=0,$con=0,$ina=0,$pivot=0){
		...
		switch($pivot){
			...
			case 3:
				$calx = $cal;
				return str_pad(floor($calx), 3, " ", STR_PAD_LEFT).' '.str_pad($conx, 2, " ", STR_PAD_LEFT).'   ';	
				break;
			default:
...
foreach ($arrAlu as $i => $value) {
	...
	if ( count($result)>0  ){
		...
		foreach ($result as $j => $value) {
			...
			// if ($result[$j]->isagrupadora_grumat == 1){
			// 	$pdf->Cell(24,4,$pdf->FormatCal(bcdiv(floor($result[$j]->cal0),'1',1),$result[$j]->con0,$result[$j]->ina0,3),'',0,'C',false);
			// 	$pdf->Cell(24,4,$pdf->FormatCal(bcdiv(floor($result[$j]->cal1),'1',1),$result[$j]->con1,$result[$j]->ina1,3),'',0,'C',true);
			// 	$pdf->Cell(24,4,$pdf->FormatCal(bcdiv(floor($result[$j]->cal2),'1',1),$result[$j]->con2,$result[$j]->ina2,3),'',0,'C',false);
			// 	$pdf->Cell(24,4,$pdf->FormatCal(bcdiv(floor($result[$j]->cal3),'1',1),$result[$j]->con3,$result[$j]->ina3,3),'',0,'C',true);
			// 	$pdf->Cell(24,4,$pdf->FormatCal(bcdiv(floor($result[$j]->cal4),'1',1),$result[$j]->con4,$result[$j]->ina4,3),'',0,'C',false);
			// }else{
			...
			// }
			// $pdf->Cell(15,4,$pdf->FormatCal( bcdiv($result[$j]->promcal,'1',1) ,$result[$j]->promcon,$result[$j]->sumina,3),'',0,'C',false);
		...
		foreach ($prom as $j => $value) {
			...
			// $pdf->Cell(15,4,$pdf->FormatCal( bcdiv($prom[$j]->promcal,'1',1),$prom[$j]->promcon,$prom[$j]->sumina,3),'',0,'C',false);
	...
	// =================================================================================
		...
		foreach ($result as $j => $value) {
			...
			$pdf->Cell(24,4,$pdf->FormatCal($result[$j]->cal4,$result[$j]->con4,$result[$j]->ina4),'',0,'C',false);
			// $pdf->Cell(15,4,$pdf->FormatCal($result[$j]->promcal,$result[$j]->promcon,$result[$j]->sumina),'',0,'C',false);
			$pdf->Cell(15,4,$pdf->FormatCal( bcdiv($result[$j]->promcal,'1',1),$result[$j]->promcon,$result[$j]->sumina),'',0,'C',false);
			$pdf->Cell(15,4,$pdf->FormatCal($result[$j]->promcalgpo,$result[$j]->promcongpo,$result[$j]->suminagpo,2),'',1,'C',true);
